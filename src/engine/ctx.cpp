...
namespace HaW
{
    ...
    )
    {
        ...
        {
            UINT64 Entry = Parser::Hooks[ i ].StubEntry;
            if ( Entry != 0 && Rip >= Entry && Rip < Entry + STUB_LENGTH )
            {
    ...
    BOOL
    Ctx::HasPrecedingCall(
        UINT64 ReturnAddress
    )
    {
        if ( ReturnAddress < MIN_VALID_ADDRESS + 8 )
        {
            return FALSE;
        }

        if ( !IsKnownImageRip( ReturnAddress ) )
        {
            return FALSE;
        }
        ...
        __try
        {
            ...
            {
                Bytes[ i ] = *( ( BYTE* )( ReturnAddress - 8 + i ) );
            }
            Ok = TRUE;
        }
